...
router.get("/getAllUsers", async(req,res)=>{
  try {
    const result = await commonController.getAll(User);
    response.successResponse(res, 200, result)
  } catch (error) {
    log.error(error);
    response.errorMsgResponse(res, 500, ERRORS.USER_NOT_FOUND)
  }
})
...
router.delete("/delete/user", async(req, res) => {
  try {
    ...
    const result = await commonController.deletePRM(User, {_id: req.params.id})
    response.successResponse(res, 200, ERRORS.USER_DELETED_SUCCESSFULLY)
  } catch (error) {
    console.log(error);
    response.errorMsgResponse(res, 500, ERRORS.USER_NOT_DELETED)
  }
})
